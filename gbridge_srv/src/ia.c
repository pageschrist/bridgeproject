...
/*
retpli_t *
explore (int prof, pli_t * pplic, int prof_max,int *nb_best,l_best_t *l_best)
{
  ...
  ligne_t ligne;
  best_t *best;
  positionc = pplic->nextpos;	
  best_score = 100000 * ((positionc) % 2) - 100000 * ((positionc + 1) % 2);
  if ((prof == prof_max) || (pplic->nopli == 13))
    {
      retpli_to_send = malloc (sizeof (retpli_t));
      retpli_to_send->score = check_plis (pplic);
      //to_send->alpha_or_beta = 0; // ce champ n'a ici pas d'importance
      return (retpli_to_send);
    }

  stk = create_stack (duplique_pli);
  nbcoups=list_all_coups (positionc, stk, pplic);
  ...
  while ((pplin = (pli_t *) pop (stk)) != NULL)
	...
	//					    clcarte].detenteur;
        ...
        //On joue le coup
        index = INDEX (positionc, nocouleur);
        pos_index = find_index (positionc,nocouleur,nocarte);
        if(pos_index==-1) {
          printf("Pb in explore \n");
          exit(1);
        }
        pos_index=remove_index(positionc,nocouleur,pos_index);
        best_cartepot.nocarte = pplin->carte[positionc].nocarte;
        best_cartepot.clcarte = pplin->carte[positionc].clcarte;
        ligne = evaluation_pli (pplin );	//on incremente nextpos ou on fixe la prochaine entame et nbpli++ 
        ...
        //On lance explore dans un thread
        retpli = explore (prof + 1, pplin, prof_max,nb_best,l_best);
        if (minimax (retpli->score, best_score, positionc))
	  {
	  best_score = retpli->score;
	  if (prof_start == prof)
	    {
	      best_carte.nocarte = best_cartepot.nocarte;
	      best_carte.clcarte = best_cartepot.clcarte;
              *nb_best=*nb_best+1;
              ...
              best->numero=*nb_best-1;
              best->carte=malloc(sizeof(carte_t));
              best->carte->nocarte=best_carte.nocarte;
              best->carte->clcarte=best_carte.clcarte;
              add_list_l(l_best,best);

              

              //Fin ajout
              //printf("Carte du moment:%s %s%s\n",affichage(positionc,POSITION),affichage(best_carte.nocarte,CARTE),affichage(best_carte.clcarte,COULEUR));
	    }
	  }
        ...
        // On annule le coup joue
        index = INDEX (positionc, nocouleur);
        if(insert_index(positionc,nocouleur,nocarte,pos_index)==0) {
          fprintf(stderr,"Pb dans insert_index\n");
          exit (EXIT_FAILURE);
        }
    ...
    vidage (stk);
    retpli_to_send = malloc (sizeof (retpli_t));
    retpli_to_send->score = best_score;
